...
def query_gpu_memory() -> Dict[str, Tuple[int, int]]:
    """Return {gpu_index: (free_mb, total_mb)} using nvidia-smi."""
    cmd = [
        "nvidia-smi",
    ...
    for line in proc.stdout.strip().splitlines():
        ...
        if len(parts) != 3:
            continue
        idx, free_mb, total_mb = parts
        try:
            info[idx] = (int(free_mb), int(total_mb))
        except ValueError:
            continue
    return info


def check_gpu_memory(gpu_list: List[str], min_free_gb: float) -> bool:
    """Print selected GPU memory and return True if all selected GPUs have enough free memory."""
    gpu_mem = query_gpu_memory()
    if not gpu_mem:
        print("[gpu-check] Could not read nvidia-smi output; skipping GPU memory preflight.")
        return True

    min_free_mb = int(float(min_free_gb) * 1024)
    bad = []
    print("[gpu-check] selected physical GPUs:")
    for gpu in gpu_list:
        if gpu not in gpu_mem:
            print(f"  GPU {gpu}: not found in nvidia-smi output")
            bad.append((gpu, None, None))
            continue
        free_mb, total_mb = gpu_mem[gpu]
        print(f"  GPU {gpu}: free={free_mb/1024:.1f} GiB / total={total_mb/1024:.1f} GiB")
        if free_mb < min_free_mb:
            bad.append((gpu, free_mb, total_mb))

    if bad:
        print("\n[gpu-check] ERROR: selected GPU(s) do not have enough free memory.")
        print(f"[gpu-check] Required at least {min_free_gb:.1f} GiB free per selected GPU.")
        print("[gpu-check] Run `nvidia-smi` and kill your old/stuck jobs or choose free GPUs.")
        print("[gpu-check] To bypass this check, pass --skip-gpu-memory-check.\n")
        return False
    return True
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=float,
        default=35.0,
        help="Minimum free GPU memory required on each selected physical GPU before launching a seed. Default: 35 GiB.",
    )
    p.add_argument(
        "--skip-gpu-memory-check",
        action="store_true",
        help="Skip nvidia-smi GPU memory preflight check.",
    )
...
def main() -> None:
    ...
    for idx, seed in enumerate(seeds):
        ...
        if args.resume_existing and test_result_path.exists():
            ...
            if not args.skip_gpu_memory_check:
                ok = check_gpu_memory(gpu_list, args.min_free_gb)
                if not ok:
                    raise RuntimeError(
                        "Selected GPU(s) are too busy for MOSEI training. "
                        "Free GPU memory first or rerun with different --gpus."
                    )
