#!/usr/bin/env bash
# ============================================================
#  MELD — AR-GOT + 8 baselines, 3 seeds, ALL 10 GPUs
#  GPU layout (100% parallel):
#    GPUs 0,1  → AR-GOT   (nproc=2, roberta-large trimodal)
...
CFG_CHADO=$PROJ/configs/meld/chado_meld_v2.yaml
...
run_chado() {
    ...
    CUDA_VISIBLE_DEVICES=0,1 torchrun \
        --nproc_per_node=2 --master_port=29820 \
        "$TRAIN_CHADO" \
...
for SEED in "${SEEDS[@]}"; do
    ...
    run_base 2  "$CFG_BPMULT"  "$SEED" "$OUT_BPMULT"  29821          & PID_BPMULT=$!
    run_base 3  "$CFG_CTNET"   "$SEED" "$OUT_CTNET"   29822          & PID_CTNET=$!
    run_base 4  "$CFG_MULT"    "$SEED" "$OUT_MULT"    29823          & PID_MULT=$!
    run_base 5  "$CFG_MMDFN"   "$SEED" "$OUT_MMDFN"   29824          & PID_MMDFN=$!
    run_base 6  "$CFG_LFLSTM"  "$SEED" "$OUT_LFLSTM"  29825          & PID_LFLSTM=$!
    run_base 7  "$CFG_AERLLM"  "$SEED" "$OUT_AERLLM"  29826          & PID_AERLLM=$!
    run_base 8  "$CFG_EMOCLIP" "$SEED" "$OUT_EMOCLIP" 29827          & PID_EMOCLIP=$!
    run_base 9  "$CFG_OVMER"   "$SEED" "$OUT_OVMER"   29828          & PID_OVMER=$!
...
echo ""
echo "============================================================"
echo "  MELD — 3-Seed Summary (AR-GOT vs All Baselines)"
echo "============================================================"
python3 - <<'PYEOF'
import json, numpy as np, os

BASE  = '/home/tahirahmad/Project_Code/CHADO_EMNLP/experiments/results/meld'
SEEDS = [42, 3407, 2024]
MODELS = [
    ('AR-GOT',   'argot_ms'),
    ('BP-MulT',  'bpmult_ms'),
...
for name, folder in MODELS:
    ...
    for s in SEEDS:
        ...
        if os.path.exists(p):
            r = json.load(open(p))
            accs.append(r['accuracy']); mf1s.append(r['macro_f1']); wf1s.append(r['weighted_f1'])
    if not accs:
